...
- **문서 버전**: v1.1
- **작성 기준**: 2026-09-18 (v1.1: 2026-09-18 AI 영역 코드 감사 반영)
- **대상**: BPM 플랫폼 리뉴얼 및 대회/리플레이/AI 통합
- **개발 원칙**: Codex 협업 가능 · SC2/Galaxy 제약 반영 · 단계적 배포
- **연관 문서**: AI 시스템(Draft AI/Gameplay AI/밸런스 테스트베드)의 구체적 설계와 현황은 [`AI_System_Improvement_Plan.md`](./AI_System_Improvement_Plan.md)를 따른다. 본 문서 §8은 개요만 유지한다.
...
> **현황 (2026-09-18 코드 감사)**: 아래 §8.1~8.4는 목표 설계이며, 현재 코드는 대부분 하드코딩된 목업/스텁 상태다(밴픽 추천은 매치업별 고정 테이블, 리플레이 분석은 가짜 통계 반환, APM/정찰/공수판단 로직 없음, AI-vs-AI 밸런스 테스트 실행기 없음). 구체적인 결함 목록과 재설계 방안, 단계별 실행 로드맵은 [`AI_System_Improvement_Plan.md`](./AI_System_Improvement_Plan.md)에 정리했다.
